    ...
    <section id="github" className="py-16 sm:py-20 lg:py-28 relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.6 }}>
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold font-[Sora] mt-2">My <span className="gradient-text">GitHub</span></h2>
        ...
        {profile && !loading && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }}
            ...
            {[
              ...
              <div key={s.label} className="flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl border border-white/10 bg-card/45 backdrop-blur-xl text-xs sm:text-sm">
        ...
        {profile && !loading && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.15 }}
            ...
            {[
              ...
              <div key={stat.label} className="rounded-2xl border border-white/10 bg-card/45 backdrop-blur-xl p-3 sm:p-5 text-center hover:border-primary/35 transition-colors">
                <p className="text-2xl sm:text-3xl font-bold font-[Sora]" style={{ color: stat.color }}>{stat.value}</p>
        ...
        {loading ? (
          <div className="mt-10 grid sm:grid-cols-2 gap-4">
            {[1,2,3,4].map(i => (
              <div key={i} className="p-5 rounded-2xl border border-white/10 bg-card/45 animate-pulse">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-10">
            {repos.map((r, i) => (
              <motion.a key={r.name} href={r.html_url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} whileHover={{ y: -3 }}
                className="group p-5 rounded-2xl border border-white/10 bg-card/45 backdrop-blur-xl hover:border-primary/35 transition-all">
